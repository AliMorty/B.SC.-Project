...
In this project, we tested the performance of two different heuristic approaches in solving an NP-Complete Problem. The problem we focus on is an extended version of the Travelling Salesman Problem. Since our approach is heuristic, there is not guarantee to find a global optimum answer. Therefore, to measure the effectiveness of our algorithm, we reduced our problem to an Integer Linear Programming Instance. For small graph examples, we used integer linear programming solvers to find the optimal solution to compare our result with the optimal one. For large graphs, since it is interactable to find the optimal solution by integer linear programming solvers, we only compared our two different methods with each other. <br>
